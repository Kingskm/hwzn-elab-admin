...
export default {
  ...
  created(){
    if(localStorage.getItem('filterRecord')){
      ...
      this.filter=filterRecord.page==='question-management'?filterRecord.filter:this.filter
  ...
  methods:{
    ...
    operationOnClick(row,operation){
      ...
      if(operation==='add'){
        ...
        localStorage.setItem('filterRecord',JSON.stringify({filter:this.filterSubmit,page:'question-management'}))
    ...
    confirm(){
      if(this.operation==='delete'){
        api_deleteQuestion(this.operationForm)
          .then((res)=>{
            if(res && res.code===0){
