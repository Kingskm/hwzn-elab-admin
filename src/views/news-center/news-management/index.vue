  ...
  <div class="container">
    ...
    <el-table ref="table" class="table" max-height="680" :data="tableData" v-loading="loading" @sort-change="sortChange">
      ...
      <el-table-column
        ...
        width="100">
        <template slot-scope="scope">
          <span :style="`color:${scope.row.status===0?'#FF0000':'#00cc00'}`">{{submitStatusEnum[scope.row.status]}}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  created(){
    if(localStorage.getItem('filterRecord')){
      ...
      this.filter=filterRecord.page==='news-management'?filterRecord.filter:this.filter
  ...
  methods:{
    ...
    operationOnClick(row,operation){
      ...
      if(operation==='add'){
        ...
        localStorage.setItem('filterRecord',JSON.stringify({filter:this.filterSubmit,page:'news-management'}))
    ...
    confirm(){
      if(this.operation==='delete'){
        api_deleteNews(this.operationForm)
          .then((res)=>{
            if(res && res.code===0){
